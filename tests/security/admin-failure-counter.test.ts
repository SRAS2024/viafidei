...
describe("admin password failure counter — Suspicious vs Breach classification", () => {
  ...
  it("the first three failures stay below threshold", () => {
  ...
  it("the FOURTH consecutive failure (more than 3) triggers Suspicious", () => {
    const args = { account: "admin", ipAddress: "1.2.3.4", deviceCredential: "dev-1" };
    for (let i = 0; i < SUSPICIOUS_FAILURE_THRESHOLD; i++) {
    ...
    expect(result.count).toBe(SUSPICIOUS_FAILURE_THRESHOLD + 1);
  });
  ...
  it("SUSPICIOUS_FAILURE_THRESHOLD is exactly 3 (rule: more than 3 in a row)", () => {
