...
export default async function JanitorEditsPage() {
  const admin = await requireAdmin();
  if (!admin) redirect("/admin/login");

  const findings = filterByAction(await scanForJanitorFindings(prisma), "edit");

  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <div>
          <h1 className="font-display text-3xl text-ink">Janitor: edits</h1>
          <p className="mt-1 font-serif text-ink-soft">
            {findings.length} published item(s) the worker recommends rebuilding.
          </p>
        </div>
          ...
          {findings.length === 0 ? (
            <tr>
              <td colSpan={5} className="px-3 py-6 text-center text-ink-faint">
                Nothing to edit — the janitor is happy.
              </td>
            </tr>
          ) : (
            findings.map((f) => (
              <tr key={f.checklistItemId} className="border-b">
                ...
                <td className="px-3 py-2 text-xs text-ink-soft max-w-md">
                  ...
                  {f.lastQaScore != null && (
                    <div className="mt-0.5">QA score: {f.lastQaScore.toFixed(2)}</div>
                  )}
                </td>
                <td className="px-3 py-2">
                  <JanitorActions itemId={f.checklistItemId} action="edit" />
                </td>
              </tr>
            ))
          )}
