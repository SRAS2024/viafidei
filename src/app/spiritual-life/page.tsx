          ...
          {guides.map((g) => {
            ...
            return (
              <article
                key={g.id}
                id={g.slug}
                className="vf-card flex flex-col rounded-sm p-8"
              >
                {staticItem ? (
                  <div className={`mb-4 ${staticItem.tone === "marian" ? "vf-icon-marian" : staticItem.tone === "eucharist" ? "vf-icon-eucharist" : "text-ink"}`}>
                    {staticItem.icon}
                  </div>
                ) : null}
