...
/**
 * Tables that the public site reads from. After the legacy-system
 * cleanup (migration 0025_drop_legacy_system) every public surface
 * reads from `PublishedContent` — there is only one public content
 * table now.
 */
export const PUBLIC_CONTENT_TABLES = ["PublishedContent"] as const;

/**
 * Every table the runtime reads or writes. Kept in sync with
 * prisma/schema.prisma and scripts/validate-db.js. Adding a model
 * means adding it here so the /api/health endpoint reports a missing
 * table instead of a generic 500 from the first request that touches it.
 */
...
/**
 * Columns whose presence has been required by a code path at least
 * once. Reported separately from "table missing" because a partially
 * applied migration is a different operator action than "no
 * migrations ran".
 */
const REQUIRED_COLUMNS: Readonly<Record<string, readonly string[]>> = {
  ...
  ChecklistItem: ["id", "contentType", "canonicalName", "canonicalSlug", "approvalStatus"],
  AdminWorkerState: ["id", "currentMode", "currentPriority", "paused", "lastHeartbeatAt"],
};
...
/**
 * Inspect Prisma's internal `_prisma_migrations` table to confirm that
 * the database has at least one applied migration and no rolled-back
 * rows. We cannot enumerate the *expected* set at runtime (the
 * prisma/migrations directory isn't shipped with the standalone
 * build), so this is a coarse health signal — the strict
 * expected-vs-applied check happens in `scripts/validate-db.js` at
 * boot, where the migrations directory is present.
 */
...
/**
 * Counts public content rows currently visible on the site. With the
 * legacy cleanup, all public content lives in `PublishedContent`
 * keyed by `(contentType, slug)`.
 */
export async function checkSeedContent(): Promise<{ ok: boolean; counts: Record<string, number> }> {
  ...
  for (const g of groups) counts[g.contentType] = g._count as number;
  const ok = Object.values(counts).some((c) => c > 0);
  return { ok, counts };
}

/**
 * Smoke-probes `PublishedContent` end-to-end through Prisma — catches
 * enum-cast and column-mismatch errors that pure metadata checks
 * (table list, column list) miss.
 */
export async function probePublicContentTables(): Promise<{
  ...
  try {
    await prisma.publishedContent.findFirst({ select: { id: true } });
  } catch (error: unknown) {
    failures.push({
