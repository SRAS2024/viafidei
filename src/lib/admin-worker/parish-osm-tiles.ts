...
export const CATHOLIC_REGIONS: Region[] = [
  ...
  // Microstates as tight boxes: a joint "Andorra & Monaco" box would span
  // all of Provence and mislabel its tiles.
  { name: "Andorra", countryCode: "AD", bbox: [42.4, 1.4, 42.7, 1.8] },
  { name: "Monaco", countryCode: "MC", bbox: [43.7, 7.3, 43.8, 7.5] },
  // ── The Americas ──────────────────────────────────────────────────────────
 ...
 * Region boxes overlap freely (a country box covers its neighbours' edges,
 * and Luxembourg sits entirely inside the Belgium/Germany/France boxes), so
 * a grid id can be generated by several regions. The FIRST occurrence fixes
 * the sweep position (dense regions first), the SMALLEST region names the
 * tile (so Luxembourg's tiles say "Luxembourg", not "Belgium"), and the
 * country-code fallback is set only when every region generating the tile
 * agrees on one code — a border tile must never stamp a church on the other
 * side with the wrong country (that would be invented data).
 */
export function osmTileCatalogue(): OsmTile[] {
  ...
  const grid = new Map<string, { tile: OsmTile; area: number; codes: Set<string | undefined> }>();
  for (const region of CATHOLIC_REGIONS) {
    const area = regionArea(region);
    for (const t of tilesFor(region)) {
      const known = grid.get(t.id);
      if (!known) {
        if (seen.has(t.id)) continue;
        seen.add(t.id);
        out.push(t);
        grid.set(t.id, { tile: t, area, codes: new Set([region.countryCode]) });
        continue;
      }
      known.codes.add(region.countryCode);
      if (area < known.area) {
        known.area = area;
        known.tile.country = region.name;
      }
    }
  }
  for (const { tile, codes } of grid.values()) {
    const [only] = codes;
    tile.countryCode = codes.size === 1 && only ? only : undefined;
  }
  catalogueCache = out;
  return out;
}
...
export async function writeTileState(
  ...
  await writeJson(prisma, TILE_PREFIX + tile.id, {
    ...
    bbox: tile.bbox,
    country: tile.country,
    // Persisted so a resumed tile keeps its country fallback (undefined for
    // border tiles — see osmTileCatalogue — and stays undefined).
    ...(tile.countryCode ? { countryCode: tile.countryCode } : {}),
    deg: tile.deg,
  });
}
...
interface CursorState {
  /** Next catalogue row to examine. */
  pos: number;
  /** Catalogue length the cursor was built for; a changed catalogue restarts. */
  size: number;
  /** When a whole lap found nothing due, sleep until the earliest nextDueAt. */
  idleUntil: number | null;
  /** Earliest nextDueAt seen during the current dry stretch. */
  minDueSeen: number | null;
  /** Consecutive catalogue rows examined without claiming anything — across
   * runs, since a run examines at most MAX_SCAN_PER_RUN rows. Reaching the
   * catalogue length means a full lap was dry, and only then does the sweep
   * park (a cursor that merely hit the end of the list has not looked at the
   * rows before its starting point). */
  dryScanned: number;
}
...
/**
 * Pick up to `max` due tiles for this run. Tiles a run stopped inside come
 * first, then split children (queue), then the cursor walks the catalogue,
 * expanding DENSE_SPLIT parents into their children and skipping tiles that
 * are not yet due. Bounded to a few hundred small reads per run; when a full
 * lap finds nothing due the sweep sleeps until the earliest nextDueAt.
 */
export async function claimDueTiles(
  ...
  const sameCatalogue = saved?.size === catalogue.length;
  const cursor: CursorState = {
    pos: sameCatalogue && typeof saved?.pos === "number" ? saved.pos : 0,
    size: catalogue.length,
    idleUntil: typeof saved?.idleUntil === "number" ? saved.idleUntil : null,
    minDueSeen: sameCatalogue && typeof saved?.minDueSeen === "number" ? saved.minDueSeen : null,
    dryScanned: sameCatalogue && typeof saved?.dryScanned === "number" ? saved.dryScanned : 0,
  };
  ...
  let scanned = 0;
  let firstClaimedPos: number | null = null;
  const pendingChildren: OsmTile[] = [];
  while (out.length < max && scanned < MAX_SCAN_PER_RUN) {
    if (cursor.dryScanned >= catalogue.length) {
      // A full lap found nothing due: park until the earliest known nextDueAt
      // (or an hour, defensively) instead of re-reading every tile each run.
      cursor.idleUntil = cursor.minDueSeen ?? now + 60 * 60 * 1000;
      cursor.minDueSeen = null;
      cursor.dryScanned = 0;
      break;
    }
    if (cursor.pos >= catalogue.length) cursor.pos = 0;
    const idx = cursor.pos;
    const tile = catalogue[idx]!;
    cursor.pos += 1;
    scanned += 1;
    const state = await readTileState(prisma, tile.id);
    if (state?.status === "DENSE_SPLIT") {
      ...
      if (kids.length > 0) {
        pendingChildren.push(...kids);
        break; // drain them (via the queue) before moving on
      }
      cursor.dryScanned += 1; // already at the minimum size: nothing more here
      continue;
    }
    if (tileIsDue(state, now)) {
      out.push({ tile, state: state ?? freshTileState() });
      if (firstClaimedPos == null) firstClaimedPos = idx;
      cursor.dryScanned = 0;
      cursor.minDueSeen = null;
    } else {
      cursor.dryScanned += 1;
      if (state?.nextDueAt != null) {
        cursor.minDueSeen =
          cursor.minDueSeen == null
            ? state.nextDueAt
            : Math.min(cursor.minDueSeen, state.nextDueAt);
      }
    }
  }
  // The cursor commits only to the FIRST tile claimed this run: the run sweeps
  // its tiles in order, so a later one may be cut off by the publish cap or the
  // deadline (or a crash) before it is written. Re-examining it next run costs
  // one memory read when it was swept, and rescues it when it was not.
  if (firstClaimedPos != null) cursor.pos = firstClaimedPos + 1;
  if (pendingChildren.length > 0) cursor.dryScanned = 0;
  ...
  return {
    ...
    detail:
      out.length > 0
        ...
        : cursor.idleUntil != null
          ? `every tile swept — parked until the earliest re-sweep`
          : `no due tile within ${scanned} catalogue rows (cursor ${cursor.pos}/${catalogue.length}, ${cursor.dryScanned} dry)`,
  };
}
