...
export async function generateAdminWorkerDeveloperAuditPdf(
  ...
  try {
    ...
    // ─── Certified Admin Skill Runtime (spec: "Worker Capability Report"):
    //     final-brain availability, certified vs missing skills, coverage, recent
    //     skill executions, and whether autonomous publishing is currently safe. ──
    if (has("Certified Admin Skill Runtime")) {
      const sk = data.skillRuntime;
      builder.section("Certified Admin Skill Runtime");
      builder.paragraph(
        "What the worker can actually do right now. The worker performs autonomous " +
          "operational work only through certified skills; anything not backed by a " +
          "certified skill is reported MISSING and a developer request is filed.",
      );
      builder.keyValue([
        { label: "Certified capabilities", value: String(sk.certified) },
        { label: "Missing capabilities", value: String(sk.missing) },
        { label: "Blocked capabilities", value: String(sk.blocked) },
        { label: "Skill executions on record", value: String(sk.totalExecutions) },
      ]);

      builder.subsection("Content type coverage");
      const buildRows = sk.rows.filter(
        (r) => r.capability.startsWith("build:") && !r.capability.includes(":", 6),
      );
      if (buildRows.length === 0) {
        builder.note("No coverage computed yet.");
      } else {
        builder.table(
          [
            { header: "Content type", weight: 200 },
            { header: "Coverage", weight: 160 },
          ],
          buildRows.map((r) => [r.contentType ?? r.capability, r.coverageStatus]),
        );
      }

      builder.subsection("Recent skill executions");
      if (sk.recentExecutions.length === 0) {
        builder.note("No skill executions recorded in this period.");
      } else {
        builder.table(
          [
            { header: "When", weight: 110 },
            { header: "Skill", weight: 150 },
            { header: "Execution", weight: 60 },
            { header: "Verify", weight: 60 },
          ],
          sk.recentExecutions
            .slice(0, 20)
            .map((e) => [
              fmtTime(e.createdAt),
              `${e.skillName}${e.contentType ? ` · ${e.contentType}` : ""}`.slice(0, 40),
              e.executionStatus,
