...
const INSTITUTION_RE =
  /\b(parish|school|hospital|church|university|college|cathedral|seminary)\b/i;
  ...
  build(ctx) {
    ...
    if (INSTITUTION_RE.test(title)) {
      return makeFailure({
        ...
        failureReason: "Candidate looks like an institution named after a saint, not a saint profile",
        candidateTitle: title,
      });
    }
    ...
    attachFieldProvenance({ ctx: internal, prov, field: "saintName", method: "title-heuristic", snippet: result.payload.saintName });
    attachFieldProvenance({ ctx: internal, prov, field: "biography", method: "body-paragraphs", snippet: result.payload.biography });
    attachFieldProvenance({ ctx: internal, prov, field: "saintType", method: "regex-classifier" });
    if (result.payload.feastDay) attachFieldProvenance({ ctx: internal, prov, field: "feastDay", method: "feast-day-regex", snippet: result.payload.feastDay });
    if (result.payload.feastMonth != null) attachFieldProvenance({ ctx: internal, prov, field: "feastMonth", method: "feast-day-regex" });
    if (result.payload.feastDayOfMonth != null) attachFieldProvenance({ ctx: internal, prov, field: "feastDayOfMonth", method: "feast-day-regex" });
    if (result.payload.patronages && result.payload.patronages.length > 0)
      attachFieldProvenance({ ctx: internal, prov, field: "patronages", method: "patronage-regex" });
    if (result.payload.officialPrayer)
      attachFieldProvenance({ ctx: internal, prov, field: "officialPrayer", method: "official-prayer-regex", snippet: result.payload.officialPrayer });
