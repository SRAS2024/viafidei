...
- **Pipeline governor — forces productive forward movement every pass.** The
  ...
  content; and when nothing downstream is productive it un-gates the keyless
  ground-truth ingest (curated + structured) so content keeps growing even when
  the brain is degraded, while running a terminal diagnostic (REPAIR → REPORTING
  → MAINTENANCE) that can never itself loop into publishing. It only changes
  **which** deterministic handler runs — every QA/publish gate is unchanged, so
  forced publishing still requires `QA_PASSED`. It never forces a stage that is
  itself spinning (so it converges instead of oscillating), never overrides a
  stage that advanced even once in the window, and is deterministic, fail-open,
  and default-on (`ADMIN_WORKER_GOVERNOR_ENABLED=0` opts out). Every override
  writes a `governor_forced_stage` log so the audit trail shows "brain chose X →
