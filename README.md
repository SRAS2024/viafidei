...
| `ADMIN_WORKER_DISCOVERY_SEEDER`                     | Keyless structured discovery seeder — queries Wikidata for devotions / Marian titles / apparitions / novenas / prayers & litanies / spiritual practices / rites and enqueues their authoritative source URLs (official websites, reference URLs) for the live extraction pipeline, so the content types with no structured ingestor still get fed authoritative sources. Discovery only (every candidate still passes extraction + verification + QA). On by default; set `0`/`false`/`off` to disable                                                                                                                                      |
...
- **Ingests structured knowledge directly — keyless, deterministic, no ceiling.**
  ...
  **self-improving** (the same row accumulates a success/failure learning signal),
  ...
  zero fabrication surface), and **DOCTOR** (Doctors of the Church). Accuracy stays
